...
# input_sheet = pd.read_excel(r'C:\Project\Adidas\Assigned Websites\LAMODA\Inputs\ASSORTMENT-INPUTS.xlsx', sheet_name='ASSORTMENT')
input_sheet = pd.read_excel(r'Assortment Input.xlsx')
input_links = input_sheet['Category URL']
input_website = input_sheet['Website']
input_country = input_sheet['Country']
input_category_id = input_sheet['Category_ID']

headers={'accept-language': 'en-US''en;q=0.9',
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36'}

def html_cache_page_saving(page_id1,cache_data):
    import datetime
    import os
    ...
    ASS_folder = f"\\\\ecxus440\\E$\\ADIDAS_SavePages\\Zalando_FR\\ASS"
...
try:
    ...
    for link, website, country, category_id in zip(input_links, input_website, input_country, input_category_id):
        cnt = 1
        for i in range(1,5):
            try:
                # p_auth = str("csimonra:h19VA2xZ")
                ...
                response = requests.get(link, headers=headers, proxies=proxy, timeout=30)
                print(response.status_code)
                if response.status_code == 200:
                    #
                    tree = html.fromstring(response.text)
                    total_pages = (tree.xpath('//span[contains(text(),"Page 1 sur")]/text()')[0]).split(' ')[-1].strip()
                    if int(total_pages) >1:
                        # for pg in range()
                        cache_data=response.text
                        page_id1 = category_id+'_1'
                        html_file_path = html_cache_page_saving(page_id1,cache_data)
                        jd1= str(response.text).split('<script type="application/json" class="re-data-el-hydrate">')[-1].split('</script><script data-re-asset>require')[0]
                        jd = json.loads(jd1)
                        m = jd['graphqlCache']
                        print(len(m))
                        keys_of_m = m.keys()
                        for k in keys_of_m:
                            ...
                            try:
                                value = m[k]
                                if 'skipHoverData":false' in str(k):
                                    try:
                                        product_url = value['data']['product']['uri']
                                    ...
                                    data_dict = {
                                        ...
                                        'Page ID': category_id+'_1',
                                        'Category_URL': link,
                                        'SKU_ID': 'Zalando_fr_'+str(SKU),
                                        'PDP URL': product_url,
                                        'Date_&_TimeStamp': datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ"),
                                        'Cache Page': html_file_path,
                                    }
                                    print(data_dict)
                                    date_time1=datetime.now().strftime("%d-%m-")
                                    data_record = data_record.append(data_dict, ignore_index=True)
                                    data_record.to_csv(f'1-{date_time1}Zalando_FR_ASS.csv', encoding='utf-8',
                        ...
                        for pg in range(2,int(total_pages)+1):
                            ...
                            for q in range(1,5):
                                ...
                                try:
                                    response1 = requests.get(url1, headers=headers, proxies=proxy1, timeout=30)
                                    if response1.status_code==200:
                                        cache_data = response1.text
                                        page_id1 = category_id + f'_{str(pg)}'
                                        html_file_path2 = html_cache_page_saving(page_id1, cache_data)
                                        jd2 = str(response1.text).split('<script type="application/json" class="re-data-el-hydrate">')[-1].split('</script><script data-re-asset>require')[0]
                                        jd3 = json.loads(jd2)
                                        m2 = jd3['graphqlCache']
                                        print(len(m2))
                                        keys_of_m2 = m2.keys()
                                        for k2 in keys_of_m2:
                                            try:
                                                value2 = m2[k2]
                                                if 'skipHoverData":false' in str(k2):
                                                    try:
                                                        product_url2 = value2['data']['product']['uri']
                                                    except Exception as e:
                                                        product_url2 = ''
                                                    try:
                                                        SKU2 = value2['data']['product']['sku']
                                                    except Exception as e:
                                                        SKU2 = ''
                                                    data_dict2 = {
                                                        'Website': 'Zalando',
                                                        ...
                                                        'Page ID': page_id1,
                                                        'Category_URL': url1,
                                                        'SKU_ID': 'Zalando_fr_' + str(SKU2),
                                                        'PDP URL': product_url2,
                                                        'Date_&_TimeStamp': datetime.now().strftime(
                                                            "%Y-%m-%dT%H:%M:%SZ"),
                                                        'Cache Page': html_file_path2,
                                                    }
                                                    print(data_dict2)
                                                    date_time1 = datetime.now().strftime("%d-%m-")
                                                    data_record = data_record.append(data_dict2, ignore_index=True)
                                                    data_record.to_csv(f'1-{date_time1}Zalando_FR_ASS.csv',
                                                                       encoding='utf-8',
                                                                       columns=['Website', 'Country', 'Category ID',
                                                                                'Page ID', 'Category_URL', 'SKU_ID',
                                                                                'PDP URL', 'Date_&_TimeStamp',
                                                                                'Cache Page'],
                                                                       index=False)
