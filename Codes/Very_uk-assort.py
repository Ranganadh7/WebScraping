...
#--------------------------------cachpage save link-----------------------------------------------
            ...
            ASS_folder = f"\\\\ecxus440\\E$\\ADIDAS_SavePages\\Very_UK\\ASS"
            ...
            totalp = general.xpath(source,'//div[@class="productsPerPage"]/span',mode='tc')
            if totalp:
                ...
                pages = round(pages) + 1
            rank = 0
            value = general.xpath(source,'//a[@class="productMainImage"]/@href',mode='set')
            if value:
                for eachval in value:
                    url = eachval
                    rank = rank + 1
                    try:
                        lastid = url.split('/')[-1].replace('.prd','')
            ...
            if int(totalp) > 1:
                ...
                for p in range(pages + 1):
                    ...
                    strurl1 = strurl + '?pageNumber=' + str(i)
