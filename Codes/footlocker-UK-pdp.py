...
headers={'authority': 'www.footlocker.co.uk',
'method': 'GET',
'path': '/',
'scheme': 'https',
...
def store_data_by_product_id(dict):
    ...
    df.to_csv("output.csv",mode="a+", encoding='utf-8-sig',index=False,header=False)
    ...
    with open('0805-FOOTLOCKER-PDP.csv', 'a+', encoding='utf-8-sig') as file:
...
def by_product_id(prod_id_data):
    ...
    # new=
    # product_id_url='https://www.footlocker.co.uk/en/product/lacoste-l001-menshoes/{}.html'.format(str(prod_id_data[0]))
    website_name = prod_id_data[0].upper()
    country = prod_id_data[1]
    sku_id = prod_id_data[2]
    product_id_url=prod_id_data[3]
    #product_id_url = 'https://www.footlocker.co.uk/en/product/nike-basketball-over-the-head-women-hoodies/317982923580.html'
    #product_id_url = 'https://footlocker.co.uk/en/product/adidas-ninja-men-hoodies/317349219680.html'
    #product_id_url = 'https://footlocker.co.uk/en/product/adidas-3-stripes-men-t-shirts/317352704180.html'

    productID=product_id_url.split('/')[-1]
    productID = productID.replace('.html', '')
    productID=str(productID)
    c_data = [website_name, country, sku_id, product_id_url]
    import datetime
    ...
    details_dict = {
        ...
        'Reason_Code': '-',
        'Crawling_TimeStamp': time.now().strftime("%Y-%m-%dT%H:%M:%SZ"),
        'Cache_Page_Link': '-',
        'Extra1': '-',
        'Extra2': '-',
        'Extra3': '-',
    ...
    try:
        for _ in range(30):
            ...
            response = requests.get(product_id_url, headers=headers, proxies=proxy1)
        ...
        if response.status_code == 200:
            ...
            #general.write_file('foot.html', response.text, 'a', encoding='UTF-8')
            tree = html.fromstring(response.content)

            all_json_data = re.findall('window.footlocker.STATE_FROM_SERVER =(.*);', response.text)

            json_loads = json.loads(all_json_data[0])
            prod_url_as_key = [key for key in json_loads['details']['data']]
            print(prod_url_as_key)

            if  prod_url_as_key:
                prod_url_as_key = prod_url_as_key[0]
                details_dict['Product_URL'] = product_id_url
                details_dict['Product_ID'] = productID
                details_dict['RPC'] = productID
                ...
                details_dict['Brand'] = brand[0].upper() + brand[1:]
                ...
                for img_no in range(1, 8):
                    image = 'https://images.footlocker.com/is/image/FLEU/{}_0{}?wid=503&hei=503&fmt=png-alpha'.format(
                        image_sku, img_no)
                    all_img.append(image)

                details_dict['Image_URLs'] = '|'.join(i for i in all_img)
                print(json_loads['details']['styles'][prod_url_as_key])
                col_grp = []
                for data in json_loads['details']['styles'][prod_url_as_key]:
                    if data['name'] in col_grp:
                        pass
                    else:
                ...
                disc1 = ''.join(val for val in disc if '\xa0' not in val)
                print((disc1, 'yes'))

                if disc1 == '' or disc1 == ' ' :
                    details_dict['Description'] = '-'
                else:
                    details_dict['Description'] = disc1

                print(details_dict['Description'] )
                ...
                details_dict['Variant_ID'] = 'Not Available'
                ...
                try:
                    ...
                    # ----
                    # ASS_folder = f"\\\\ecxus440\\E$\\ADIDAS_SavePages\\Footlocker.uk\SON"
                    ASS_folder = f"\\\\ecxus440\\E$\\ADIDAS_SavePages\\Footlocker_uk\\PDP"
                ...
                for element in size_info_html:
                    ...
                    if 'out of stock' in ob2[0]:
                        ...
                        details_dict['Stock_Condition'] = 'InStock'
                ...
                prod_url_as_key = [key for key in json_loads['details']['data']]
                print(prod_url_as_key, 'No')
                print("Product Not Found or Sold Out")
                details_dict['Stock_Condition'] = "Out of Stock"
                details_dict['Product_URL'] = product_id_url
            ...
            print(('Request Failed with status code of ', response.status_code))
            details_dict['Reason_Code'] = 'Success-PNF'
            store_data_by_product_id(dict=[details_dict])
            Req_Failed_arr.append(c_data)
...
header = ['Website', 'Country', 'SKU_ID', 'PDP_URL']
...
details_dict_1 = {
        ...
        'Colour_of_Variant': 'Colour_of_Variant',
        'Colour_Grouping': 'Colour_Grouping',
        ...
        'Cache_Page_Link': 'Cache_Page_Link',
        'Extra1': 'Extra1',
        'Extra2': 'Extra2',
        'Extra3': 'Extra3',
        'Extra4': 'Extra4',
...
Req_Failed_arr = []
store_data_by_product_id(dict=[details_dict_1])
...
df = pd.DataFrame(Req_Failed_arr, columns=['Website', 'Country', 'SKU_ID', 'PDP_URL'])
df.to_csv('Request_Failed_Urls.csv', header=False, index=False)
