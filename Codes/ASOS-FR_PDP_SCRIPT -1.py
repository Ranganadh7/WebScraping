...
'referer': 'https://www.asos.com/fr/mujer/',
...
def store_data_by_product_id(dict):
    ...
    with open('output1-0504.csv', 'a+', encoding='utf-8-sig', newline='') as file:
...
def html_cache_page_saving(sku,saving_data):
    ...
    #cpid=pageid+'_'+'29_04_2022'

    ASS_folder = f"\\\\ecxus440\\E$\\ADIDAS_SavePages\\Asos_FR\\PDP"
...
dom='https://www.asos.com'
final_array = []
def by_product_id(prod_id_data,n):
    product_id_url=prod_id_data[0]
    print(product_id_url)
    INPUT_url = prod_id_data[0]
    # INPUT_url = 'https://www.asos.com/fr/adidas/adidas-originals-survetement-gris/grp/37418?clr=gris&cid=5906'
    print(INPUT_url)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                if cat !=[]:
                    ...
                    cat = str(response.text).split("breadcrumb: '")[-1].split("version: '")[0].strip().strip("',")
                    Category_Path = cat.replace('//','>')
                #print(Category_Path)
                ...
                try:
                    ...
                    pri = str(response.text).split("config.stockPriceApiUrl = '")[-1].split('/*~stockPrice~*/')[0]
                    price_link = 'https://www.asos.com'+str(pri).replace("'",'').replace(";","").strip()
                    # price_link = f'https://www.asos.com/api/product/catalogue/v3/stockprice?productIds={product_id_specific}&store=ES&currency=EUR&keyStoreDataversion=hgk0y12-29'
                    ...
                    try:
                        List_price = price_json[1]['productPrice']['rrp']['text']
                    except:
                        List_price =price_json[0]['productPrice']['previous']['text']
                    # Promo_price = price_json[0]['productPrice']['current']['text']
                    try:
                        Promo_price = price_json[1]['productPrice']['current']['text']
                ...
                try:
                   # disurlfr = f'https://www.asos.com/api/product/catalogue/v3/stockprice?productIds={product_id_specific}&store=DE&currency=EUR&keyStoreDataversion=hgk0y12-29'
                    ...
                    try:
                        z = price_json[1]['discountPercentage']
                    except:
                        z = dis_json[0]['discountPercentage']
                    dis = (str(z) + '%')
                    ...
                    #details_dict['List_Price']=tree.xpath('//span[@class="product-prev-price"][2]/text()')
                    #details_dict['Promo_Price']=tree.xpath('//span[@data-id="current-price"]/text()')
                ...
                try:
                    ...
                    var = len(product_json['variants'])
                    if var == 0:
                        var = len(product_json['products'][0]['variants'])
                        if var >0:
                            for v in range(0, var):
                                vars = product_json['products'][0]['variants'][v]['size']
                                stock = product_json['products'][0]['variants'][v]['isInStock']
                                vid = product_json['products'][0]['variants'][v]['variantId']
                                Seller_Name = product_json['products'][0]['variants'][v]['seller']
                                if stock == True:
                                    ...
                                    print(details_dict)
                                    # final_array.append(details_dict)
                        ...
                        for v in range(0, var):
                            ...
                            Seller_Name = product_json['variants'][v]['seller']
                            if stock == True:
                                details_dict['Variant']=general.clean(vars)
                                details_dict['Variant_ID'] = vid
                                details_dict['Stock_Condition']="In Stock"
        ...
        if n != 0:
            ...
            print(details_dict)
            store_data_by_product_id(dict=[details_dict])
    # data_df = pd.DataFrame(final_array)
    # data_csv = data_df.to_csv(r"output-pdp-p-1.csv",encoding="utf-8-sig", index=False)
